...
Inkset ships with a default stylesheet. Colors and fonts come from CSS custom properties on the `<Inkset>` root, so you can theme it inline, in your stylesheet, or by opting into `unstyled` and owning the visual layer yourself.
...
.inkset-root {
  ...
  --inkset-color-text-muted: #6a6a6a;
  --inkset-heading-1-size: 2em;
  --inkset-heading-1-weight: 700;
  --inkset-code-block-bg: #f4f4f4;
}
```
