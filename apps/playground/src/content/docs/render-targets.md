...
The render layer takes the positioned layout tree and writes it to a target: DOM, canvas, or a custom destination.
...
The core's layout output is framework-agnostic — a flat list of positioned blocks. A canvas renderer could walk the list and draw to a `<canvas>`. A PDF renderer could write to PDFKit. Neither ships in v1; both are possible.
